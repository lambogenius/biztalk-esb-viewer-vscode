...
const viewType = 'biztalkEsbViewer.editor';
...
function activate(context) {
  ...
  const command = vscode.commands.registerCommand('biztalkEsbViewer.open', async uri => {
    const target = uri || (await vscode.window.showOpenDialog({
      canSelectMany: false,
      filters: { 'BizTalk ESB definitions': ['xml', 'esb'] },
    }))?.[0];
